...
mod rust_crypto {
    ...
    use crate::{common::headers::Writer, dkim::Canonicalization};

    use super::{
        Algorithm, Error, HashContext, HashImpl, HashOutput, Result, Sha1, Sha256, SigningKey,
        VerifyingKey,
    };
    ...
    impl RsaPublicKey {
        pub(crate) fn verifying_key_from_bytes(
            ...
            Ok(Box::new(RsaPublicKey {
                inner: <rsa::RsaPublicKey as rsa::pkcs8::DecodePublicKey>::from_public_key_der(
                    bytes,
                )
